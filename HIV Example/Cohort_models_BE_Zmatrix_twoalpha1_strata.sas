/*
	file : h:\PCORI-SW\Paper3-Tutorial\Cohortdata\Cohort_models_BE_Zmatrix_twoalpha1_strata.sas
	name : John Preisser
	date: 11/12/2024
    Modified: John Preisser
    what: run GEE/MAEE for cohort example in tutorial paper - HIV analysis Table 6, with strata

    degrees of freedom of test and CIs, t-dist with I-2 = 8-2 = 6

    input: h:\PCORI-SW\Paper3-Tutorial\Cohortdata\hivtesting.sas7bdat
    output: h:\PCORI-SW\Paper3-Tutorial\Cohortdata\Cohort_models_BE_Zmatrix_twoalpha1.rtf
*/
...
ods rtf style=journal file="H:\PCORI-SW\Paper3-Tutorial\Cohortdata\Cohort_models_BE_Zmatrix_twoalpha1_strata.rtf";
...
proc sql;
	...
	select
		...
		(a1.clusternum = a2.clusternum and a1.ID ne a2.ID and a1.time ne a2.time) as z2,
		(a1.clusternum = a2.clusternum and a1.ID = a2.ID) as z3
...
* add PRINTRANGE=NO to suppress printing range violations except when variance computed;
 ...
 title "GEE/MAEE estimation of logistic model for binary outcome hiv testing. Model 2: Modified BE correlation with user-defined z-matrix and identity link";
%GEEMAEE(xydata=a, yvar= hivt, ytype = binary, link =logit, xvar= Shandong period1 period2 period3 period4 intervention, maxiter=100, PRINTRANGE=NO, corrlink = identity, df_choice=2,
    clusterID= clusternum, Zdata=zd, zvar = z1 z3, zpair = zpair1 zpair2, makevone=NO, makephione=YES, alpadj=MAEE, ESTOUT=parmests2, VAR_BETA=BC1, VAR_ALPHA=BC2);
...
data conflimits;
  ...
  if not(_name_="ICC_parm1") and not(_name_="ICC_parm2") then do;
     OR_est = exp(estimate);
