...
SAS Macro GEEMAEE version 2.04

filename: MAEEV2.04.sas
...
# version 2.03 by Oct, 2024 by Ying Zhang, correct the Correlation structures errors when specify BE and PD
# version 2.04 by Nov, 2024 by Ying Zhang, Edit the corrlink to be flexible (with LOG or the others), rather than just identity for EX, BE, NE
################ also adding the degree of freedom option of I-2 rather than I-p/I-q for the beta and alpha testing results
**************************************************************************************************************
...
printfinal: Print final results; 1=yes, 0=no, optional, default is yes.           
  ...
  makephione: Set the dispersion parameter as 1 
    ...
    df_choice: for Degrees of freedom, value =1: df = I − p/I-q is default; p is number of mean model parameters and q is the number of the ICCs
				value= 2: df = I − 2
...
maxiter=50, epsilon=0.0001, printrange=YES, df_choice=1,
		   ...
		   %put NOTE: Correlation structure is EX;
		     do i = 1 to nrow(n);
		      ...
		      if n[i]>1 then do; 
		   	...
		   	%if %length(&corrlink)=0 %then %let corrlink = IDENTITY;
     %end; 
 %else %if %upcase(&Corr)=PD %then %do;
	%put NOTE: Correlation structure is PD;
            use &xydata NOBS nobs;
			         read all var{&periodID} into periodid;
			     ...
			     do i = 1 to nrow(n);
			   	 ...
			   	 %let corrlink = LOG;
	%end; 
%else %if %upcase(&Corr)=BE %then %do;
	   %put NOTE: Correlation structure is BE;
	      use &xydata NOBS nobs;
	         read all var{&periodID} into periodid;
	         read all var{&subjectID} into subjectid;
	      close &xydata;
...
%else %if %upcase(&Corr)=ED %then %do;
		   ...
		   %put NOTE: Correlation structure is ED;
		     ...
		     do i = 1 to nrow(n);  
		   	 ...
		   	 %let corrlink = LOG;
	%end;  
 %else %if %upcase(&Corr) =NE %then %do;
 	%put NOTE: Correlation structure is NE;
		      use &xydata NOBS nobs;
		         read all var{&periodID} into periodid;
		      close &xydata;
...
start linkid(lp); 
    ...
    %ELSE %IF %upcase(&link) = LOG %THEN %DO;
            return(exp(lp)); 
    %END;
    
    %ELSE %IF %upcase(&link) = LOGIT %THEN %DO;
            return(exp(lp)/(1+exp(lp))); 
    %END;

	%ELSE %IF %upcase(&link) = PROBIT %THEN %DO; 
            return(probnorm(lp));
    %END;
finish linkid;
...
**************************************************************************************/
   ...
   start CREATEC(X, mu);
    ...
    %ELSE %IF %upcase(&link) = LOG %THEN %DO; /*log link*/
        return(X#mu);  
    %END;
    
    %ELSE %IF %upcase(&link)= LOGIT %THEN %DO; /*logit link*/;
        return(X#(mu#(1-mu)));  
    %END;

	%ELSE %IF %upcase(&link) = PROBIT %THEN %DO; /*probit link*/;
...
start corrlinkid(lp2); 
    ...
    %ELSE %IF %upcase(&corrlink) = LOG %THEN %DO;
            return(exp(lp2)); 
    %END;

    %ELSE %IF %upcase(&corrlink) = LOGIT %THEN %DO;
            rhoi = exp(lp2); 
            return(rhoi/(1+rhoi)); 
    %END;

	%ELSE %IF %upcase(&corrlink) = FISHERSZ %THEN %DO;
...
**************************************************************************************/
 ...
 if   printfinal =1 then do;   
	 ...
	 * set the option of I-2 or I-q;
	%if &df_choice=1 %then %do;
	%put NOTE: degree of freedom is &df_choice for beta;
	    ...
	    %put NOTE: degree of freedom is &df_choice for beta;
	      bsebc0 = sqrt(vecdiag(robust[1:p, 1:p]));
	      Tbeta_bc0=beta / bsebc0;
	      bpval0 = 2#(1 - probt(abs(Tbeta_bc0), I-2));
	
	      bsebc1 = sqrt(vecdiag(varKC[1:p, 1:p]));
	      Tbeta_bc1 = beta / bsebc1;
	      bpval1 =2#(1 - probt(abs(Tbeta_bc1), I-2));
	
	      bsebc2 = sqrt(vecdiag(varMD[1:p, 1:p]));
	      Tbeta_bc2 = beta / bsebc2;
	      bpval2 =2#(1 - probt(abs(Tbeta_bc2), I-2));
	      
	      bsebc3 = sqrt(vecdiag(varFG[1:p, 1:p]));
	      Tbeta_bc3 = beta / bsebc3;
	      bpval3 =2#(1 - probt(abs(Tbeta_bc3), I-2));
	    
	      stderr = sqrt(vecdiag(naive));
	      Tnai = beta / stderr;
	      pvalue =2#(1 - probt(abs(Tnai), I-2));
	    %end;
	...
	* set the option of I-2 or I-q;
	%if &df_choice=1 %then %do;
	%put NOTE: degree of freedom is &df_choice for ICCs;
	...
	%end; %else %do;
	      %put NOTE: degree of freedom is &df_choice for ICCs;
	       asebc0 = sqrt(vecdiag(robust[p+1:p+q, p+1:p+q]));
	      Talpha_bc0 = alpha / asebc0;
	      apval0 = 2#(1 - probt(abs(Talpha_bc0), I-2));
	
	      asebc1 = sqrt(vecdiag(varKC[p+1:p+q, p+1:p+q]));
	      Talpha_bc1 = alpha / asebc1;
	      apval1 =2#(1 - probt(abs(Talpha_bc1), I-2));
	
	      asebc2 = sqrt(vecdiag(varMD[p+1:p+q, p+1:p+q]));
	      Talpha_bc2 = alpha / asebc2;
	      apval2 =2#(1 - probt(abs(Talpha_bc2), I-2));
	      
	      asebc3 = sqrt(vecdiag(varFG[p+1:p+q, p+1:p+q]));
	      Talpha_bc3 = alpha / asebc3;
	      apval3 =2#(1 - probt(abs(Talpha_bc3), I-2));
	%end;
